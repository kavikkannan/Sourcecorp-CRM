...
export default function UserManagement() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (role) {
        try {
          const response = await fetch(`http://localhost:9999/api/users/${role}`);
  ...
  useEffect(() => {
    const fetchAllUsers = async () => {
      try {
        const response = await fetch("http://localhost:9999/api/all-users");
  ...
  const handleAppoint = async (UserID, AppointedUserID) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:9999/api/appoint", {
        method: "POST",
        mode: "cors",
